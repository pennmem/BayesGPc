...
C not used successfully, just another attempt to use C++/C in Fortran
C interface
C     subroutine FCN(N, THETA, H) bind(C, name="FCN")
C         use iso_c_binding
C         implicit none
C         type(c_int), value :: N
C         type(c_ptr), value :: THETA
C         type(c_ptr), value :: H
C     end subroutine
C end interface

C       SUBROUTINE FCN(N,THETA,H)
C C  This subroutine is from the example in Judge et al., The Theory and
C C  Practice of Econometrics, 2nd ed., pp. 956-7. There are two optima:
C C  F(.864,1.23) = 16.0817 (the global minumum) and F(2.35,-.319) = 20.9805.
C 
C       DOUBLE PRECISION THETA(2), H
C 
C       DOUBLE PRECISION Y(20), X2(20), X3(20)
C 
C       Y(1) = 4.284
C       Y(2) = 4.149
C       Y(3) = 3.877
...
C       H = 0.0
C       DO 100, I = 1, 20
C          H = (THETA(1) + THETA(2)*X2(I) + (THETA(2)**2)*X3(I) - Y(I))**2
C      1       + H
C 100   CONTINUE
C 
C       RETURN
